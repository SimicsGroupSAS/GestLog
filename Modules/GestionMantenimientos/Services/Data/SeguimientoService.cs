...
namespace GestLog.Modules.GestionMantenimientos.Services.Data
{
    public class SeguimientoService : ISeguimientoService
    {
        ...
        {
            _logger.LogInformation("[SeguimientoService] Iniciando importaciÃ³n de seguimientos antiguos: {FilePath}", filePath);
            try
            {
                ...
                // Mapeo flexible de encabezados: esperado -> posiciÃ³n de columna
                var columnMap = MapearColumnasExcel(worksheet);

                // Obtener lista de cÃ³digos vÃ¡lidos de equipos
                ...
                // Obtener el rango usado del worksheet para saber cuÃ¡ntas filas hay
                var rangeUsed = worksheet.RangeUsed();
                int lastRow = rangeUsed?.LastRow().RowNumber() ?? 1;

                _logger.LogInformation("[SeguimientoService] Excel detectado: Ãšltima fila con datos = {LastRow}", lastRow);

                using var dbContext = _dbContextFactory.CreateDbContext();

                // Iterar desde la fila 2 hasta la Ãºltima fila usada del archivo (NO solo hasta encontrar una vacÃ­a)
                for (int row = 2; row <= lastRow; row++)
                {
                    try
                    {
                        ...
                        {
                            string razon = $"Equipo con cÃ³digo '{codigo}' no existe";
                        ...
                        {
                            string razon = $"Tipo de mantenimiento '{tipoMtnoStr}' no es vÃ¡lido (solo Preventivo o Correctivo)";
                            seguimientosIgnorados.Add((row, razon));
                            _logger.LogWarning("[SeguimientoService] Fila {Row} - Campos: Codigo='{Codigo}', Nombre='{Nombre}', TipoMtno='{TipoMtno}' | Rechazo: {Razon}",
                                row, codigo, nombre, tipoMtnoStr, razon);
                            continue;
                        ...
                        {
                            string razon = "Fecha RealizaciÃ³n no es una fecha vÃ¡lida";
                            string fechaStr = worksheet.Cell(row, columnMap["FechaRealizacion"]).GetString()?.Trim() ?? "(vacÃ­a)";
                            seguimientosIgnorados.Add((row, razon));
                        ...
                        // Calcular semana y aÃ±o a partir de FechaRealizacion
                        var cal = System.Globalization.CultureInfo.CurrentCulture.Calendar;
                        ...
                        var dto = new SeguimientoMantenimientoDto
                        {
                            ...
                            FechaRegistro = fechaRealizacion, // Usar la misma fecha de realizaciÃ³n
                    ...
                    catch (GestionMantenimientosDomainException ex)
                    {
                        ...
                        _logger.LogWarning("[SeguimientoService] Error de validaciÃ³n en importaciÃ³n - Fila {Row}: {Message}", row, ex.Message);
                    }
                    catch (Exception ex)
                    {
                        seguimientosIgnorados.Add((row, ex.Message));
                        _logger.LogWarning(ex, "[SeguimientoService] Error inesperado en importaciÃ³n - Fila {Row}", row);
                    }
                }                // Guardar todos los seguimientos vÃ¡lidos
                if (seguimientosImportados.Any())
                {
                    ...
                    foreach (var seg in seguimientosImportados)
                    {
                        ...
                        {
                            // âœï¸ ACTUALIZAR: Solo si AMBOS son Preventivos
                        ...
                        {
                            // ðŸ”’ IGNORAR: Los Correctivos NUNCA se actualizan (son independientes)
                    ...
                    _logger.LogInformation("[SeguimientoService] ImportaciÃ³n completada: Nuevos={Nuevos}, Actualizados={Actualizados}, Correctivos ignorados={IgnoradosCorrectivos}", 
                        seguimientosImportados.Count - actualizados - preventivosIgnorados, 
                        actualizados, 
                        preventivosIgnorados);
                }

                _logger.LogInformation("[SeguimientoService] ImportaciÃ³n completada: {Importados} importados, {Ignorados} ignorados",
                    seguimientosImportados.Count, seguimientosIgnorados.Count);

                // Crear cronogramas automÃ¡ticamente desde los seguimientos importados
                ...
                // Log de ignored items para diagnÃ³stico
            ...
            {
                _logger.LogError(ex, "[SeguimientoService] Error de validaciÃ³n en importaciÃ³n");
                throw;
            }
            catch (Exception ex)
            {
                ...
                throw new GestionMantenimientosDomainException(
                    ...
                    $"\n- Datos vÃ¡lidos a partir de la fila 2" +
        ...
        /// <summary>
        /// Obtiene la lista de cÃ³digos vÃ¡lidos de equipos existentes en la BD
        /// </summary>
        ...
        /// <summary>
        /// Mapea flexiblemente los encabezados del Excel (ignora espacios, mayÃºsculas, caracteres especiales)
        /// Retorna un diccionario con el nombre lÃ³gico -> nÃºmero de columna
        /// </summary>
        private Dictionary<string, int> MapearColumnasExcel(IXLWorksheet worksheet)
        {
            ...
            // Normalizar funciÃ³n: quita espacios, caracteres especiales y convierte a minÃºsculas
            ...
            {
                throw new GestionMantenimientosDomainException(
                    "El archivo Excel estÃ¡ vacÃ­o. Por favor, carga un archivo con datos.");
            }
            ...
            foreach (var colRequerida in columnasRequeridas)
            {
                ...
                {
                    throw new GestionMantenimientosDomainException(
                        $"No se encontrÃ³ la columna '{colRequerida}' en el Excel. " +
                        $"AsegÃºrate de que el archivo tenga las siguientes columnas: " +
                        $"Codigo, Nombre, TipoMtno, Descripcion, Responsable, Costo, Observaciones, FechaRealizacion");
                }
            }
