...
{    public partial class DetallesEquipoInformaticoView : Window
    {
        ...
        }        private async void BtnEditar_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (result == true && editarWindow.DataContext is GestLog.Modules.GestionEquiposInformaticos.ViewModels.Equipos.AgregarEquipoInformaticoViewModel editVm2)
                {
                    try
                    {
        ...
        {            try
            {
                if (!(this.DataContext is GestLog.Modules.GestionEquiposInformaticos.ViewModels.Equipos.DetallesEquipoInformaticoViewModel vm))
                {
                    System.Windows.MessageBox.Show("No se pudo obtener la información del equipo.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
